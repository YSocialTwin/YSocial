...
def ensure_kernel_installed(kernel_name="python3_ysocial"):
    ...
    try:
        ...
        if is_frozen:
            ...
            print("Running from PyInstaller bundle - using system Python for Jupyter kernel")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if kernel_name in kernels:
                        ...
                        print(f"Using existing 'python3' kernel (custom kernel '{kernel_name}' not needed in PyInstaller mode)")
                        return True
                    else:
                        # Try to install ipykernel in system Python if no kernel exists
                        print(f"No Python kernel found. Installing ipykernel in system Python...")
                        subprocess.run(
                        ...
                        subprocess.run(
                            [python_exe, "-m", "ipykernel", "install", "--user", "--name", "python3"],
                            capture_output=True,
                            check=True,
                        )
                    ...
                    print(f"Warning: jupyter not available in system Python. Jupyter Lab may not work correctly.")
                    print(f"Please install jupyter and ipykernel: pip install jupyterlab ipykernel")
                    return False
                    
            except (subprocess.CalledProcessError, subprocess.TimeoutExpired, json.JSONDecodeError) as e:
                print(f"Warning: Could not verify Jupyter installation in system Python: {e}")
                print(f"Please ensure jupyterlab and ipykernel are installed in your system Python")
                return False
        else:

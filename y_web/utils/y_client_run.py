...
def main():
    ...
    parser.add_argument(
        "-c", "--config", required=True, help="Path to client configuration JSON file"
    )
    ...
    parser.add_argument(
        "--population-id", required=True, help="Population ID", type=int
    )
    parser.add_argument(
        "--resume", action="store_true", default=True, help="Resume from last state"
    )
    parser.add_argument(
    ...
    from y_web.models import Client, Client_Execution, Experiment, Population
    ...
    with app.app_context():
        ...
        population = (
            db.session.query(Population).filter_by(id=args.population_id).first()
        )
        if not population:

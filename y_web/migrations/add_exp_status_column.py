"""
Database migration script to add exp_status column to exps table.

This script adds:
- exp_status: String column (default 'stopped')
  Values: 'stopped', 'active', 'completed', 'scheduled'
...
def migrate_sqlite(db_path):
    """
    Add exp_status column to SQLite database.
    ...
    try:
        ...
        if "exp_status" not in columns:
            cursor.execute(
                "ALTER TABLE exps ADD COLUMN exp_status TEXT DEFAULT 'stopped'"
            )
            ...
            print("✓ Updated exp_status for existing experiments")
        else:
            print("○ exp_status column already exists in SQLite database")
...
def migrate_postgresql(host, port, database, user, password):
    """
    Add exp_status column to PostgreSQL database.
    ...
    try:
        ...
        if "exp_status" not in columns:
            ...
            print("○ exp_status column already exists in PostgreSQL database")
...
def main():
    ...
    print("YSocial Database Migration: Adding Experiment Status Column")
    print("=" * 60)
    print()

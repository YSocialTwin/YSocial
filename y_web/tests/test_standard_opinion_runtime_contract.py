...
def test_base_agent_restores_opinion_runtime_path():
    ...
    assert 'self._record_self_post_opinions(topic_ids=interests_id, tid=int(tid))' in source
...
def test_client_web_passes_experiment_db_path_and_opinions():
    ...
    assert 'experiment_db_path=os.path.join(self.base_path, "database_server.db")' in source
    assert "def _rule_based_agents_enabled(self):" in source
    assert 'AgentClass = FakeAgent if self._rule_based_agents_enabled() else Agent' in source
    assert 'PageClass = FakePageAgent if self._rule_based_agents_enabled() else PageAgent' in source
...
def test_generate_user_uses_fake_agent_for_non_llm_configs():
    ...
    assert "AgentClass = FakeAgent if _rule_based_agents_enabled(config) else Agent" in source
    assert "PageClass = FakePageAgent if _rule_based_agents_enabled(config) else PageAgent" in source

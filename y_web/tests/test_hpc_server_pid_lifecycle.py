...
def test_stop_hpc_server_force_terminates_process_tree(monkeypatch):
    ...
    monkeypatch.setattr(
        hpc_server, "_clear_stale_hpc_server_pid", lambda current, exp_folder=None: False
    )
    monkeypatch.setattr(hpc_server, "_tracked_process_is_alive", lambda pid: True)
    ...
    with (
        patch("y_web.src.simulation.port_manager._force_terminate_process_tree") as force_kill,

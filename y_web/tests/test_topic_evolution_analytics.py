...
def test_hashtag_evolution_tracks_volume_population_share_and_lifecycle(tmp_path):
    db_path = tmp_path / "hashtag_evolution.db"
    with sqlite3.connect(db_path) as conn:
        conn.executescript("""
            CREATE TABLE user_mgmt (
                id TEXT PRIMARY KEY,
    ...
    analytics = opinion_module._build_hashtag_evolution_payload(
        str(db_path),
        filter_day=3,
        filter_hour=1,
    ...
    assert analytics["page_title"] == "Hashtag Evolution"
    assert analytics["selector_label"] == "Hashtags to Visualize"
    assert analytics["distribution"]["labels"] == ["#ClimateNow", "#RoboticsFuture"]
    assert analytics["distribution"]["datasets"][0]["data"] == [5, 1]
    assert analytics["trend"]["type"] == "heatmap"
    assert analytics["secondary"]["type"] == "heatmap"
    ...
    assert reach_day_2["percent"] == 75.0
    assert analytics["topic_lifecycle"]["datasets"][0]["data"] == [1, 1, 1]
    assert analytics["summary"]["rows"][0][0] == "#ClimateNow"
    assert analytics["summary"]["rows"][0][3] == 5

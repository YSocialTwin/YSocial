...
def test_interview_routes_and_helpers_support_uuid_user_ids():
    routes = Path(
        "/app/y_web/routes/api/interview/_routes.py"
    ).read_text(encoding="utf-8")
    helpers = Path(
        "/app/y_web/routes/api/interview/_helpers.py"
    ).read_text(encoding="utf-8")
    memory = Path(
        "/app/y_web/routes/api/interview/_memory.py"
    ).read_text(encoding="utf-8")
    facts = Path(
        "/app/y_web/routes/api/interview/_facts.py"
    ).read_text(encoding="utf-8")
...
def test_interview_frontend_preserves_uuid_agent_ids():
    script = Path(
        "/app/y_web/static/assets/js/interview.js"
    ).read_text(encoding="utf-8")
...
def test_interview_unavailable_memory_snapshot_preserves_uuid_agent_ids():
    server = Path(
        "/app/y_web/routes/api/interview/_server.py"
    ).read_text(encoding="utf-8")

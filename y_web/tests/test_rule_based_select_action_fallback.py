...
def test_forum_rule_based_runtime_uses_fake_agents():
    ...
    assert 'self.config.setdefault("agents", {})["llm_agents"] = [None]' in client_source
    assert 'self._coerce_legacy_rule_based_config(data.get("agents", []))' in client_source
    assert 'self._coerce_legacy_rule_based_config(agents.get("agents", []))' in client_source
    ...
    assert "AgentClass = FakeAgent if _rule_based_agents_enabled(config) else Agent" in utils_source
...
def test_forum_rule_based_fake_agent_records_opinion_updates():
    ...
    assert "self._record_self_post_opinions(topic_ids=interests_id, tid=int(tid))" in fake_source
    ...
    assert 'def comment_image(self, image: object, tid: int, article_id: int = None):' in fake_source
...
def test_forum_web_init_restores_fake_agent_follow_probabilities():
    ...
    assert 'config["agents"].get("probability_of_daily_follow", 0)' in reddit_base_source
    assert "self.probability_of_secondary_follow = float(" in reddit_base_source
    assert 'config["agents"].get("probability_of_secondary_follow", 0)' in reddit_base_source
...
def test_forum_fake_agent_uses_runtime_db_session_for_news_and_images():
    ...
    assert "from y_client.clients.client_web import session as global_session" in fake_source
...
def test_forum_new_opinions_resolves_numeric_author_id():
    ...
    assert "author_id, _author_username = self.get_username_from_post(post_id)" in reddit_base_source

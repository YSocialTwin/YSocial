...
def _resolve_experiment_actor(user):
    ...
    if username:
        exp_user = db.session.scalars(
            select(User_mgmt).filter_by(username=username)
        ).first()
...
def create_comment_reddit(
    ...
    try:
        ...
        sentiment_parent = ""
        parent_sentiment = db.session.scalars(
            select(Post_Sentiment).filter_by(post_id=parent_id)
        ).first()
        ...
        admin_user = db.session.scalars(
            select(Admin_users).filter_by(username=actor_user.username)
        ).first()
        llm = "llama3.2:latest"
        ...
        for emotion_name in emotions:
            if len(emotion_name) < 1:
                continue
            emotion = db.session.scalars(
                select(Emotions).filter_by(emotion=emotion_name)
            ).first()
        ...
        for tag in hashtags:
            if len(tag) < 4:
                continue
            hashtag = db.session.scalars(
                select(Hashtags).filter_by(hashtag=tag)
            ).first()
            if hashtag is None:
        ...
        for mention in mentions:
            if len(mention) < 1:
                continue
            mentioned_user = db.session.scalars(
    ...
    try:
        if normalized_url:
            ...
            if looks_like_media:
                ...
                img = db.session.scalars(
                    select(Images).filter_by(url=stored_url)
                ).first()
                if img is None:
                    img = Images(
                ...
                existing_article = db.session.scalars(
                    select(Articles).filter_by(link=normalized_url)
                ).first()
                if existing_article:
                    ...
                    website = db.session.scalars(
                        select(Websites).filter_by(name=source)
                    ).first()
                    if not website:
                        website = Websites(
                            name=source[:50],
                    ...
                    if image_url and len(image_url) <= 200:
                        existing_image = db.session.scalars(
                            select(Images).filter_by(article_id=article.id)
                        ).first()
                        if existing_image is None:
                            existing_image = db.session.scalars(
                                select(Images).filter_by(url=image_url)
                            ).first()
                        if existing_image is None:
        ...
        for topic_name in topics:
            interest = db.session.scalars(
                select(Interests).filter_by(interest=topic_name)
            ).first()
            if interest is None:

...
def validate_spec_file():
    ...
    if (
        "a.binaries," in exe_section
        and "a.zipfiles," in exe_section
        and "a.datas," in exe_section
    ):
        print(
            "  ❌ EXE still includes a.binaries, a.zipfiles, a.datas (single-file mode)"
        )
        return False
...
def validate_macos_scripts():
    ...
    if 'if [ ! -d "dist/YSocial_dist" ]' not in build_content:
        print(
            "  ❌ build_and_package_macos.sh doesn't check for dist/YSocial_dist directory"
        )
        return False
...
def validate_documentation():
    ...
    if (
        "multi-file" not in build_content.lower()
        and "onedir" not in build_content.lower()
    ):
